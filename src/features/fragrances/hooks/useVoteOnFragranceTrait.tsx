...
export const useVoteOnFragranceTrait = () => {
  const [voteOnFragranceTraitInner] = useMutation(VOTE_ON_FRAGRANCE_TRAIT_MUTATION)

  const voteOnFragranceTrait = (input: VoteOnFragranceTraitInput) => {
    return wrapQuery(voteOnFragranceTraitInner({ variables: { input } })).map(data => data.voteOnFragranceTrait)
  }

  return { voteOnFragranceTrait }
}