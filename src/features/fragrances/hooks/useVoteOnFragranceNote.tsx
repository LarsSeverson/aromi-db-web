...
export const useVoteOnFragranceNote = () => {
  const [voteOnFragranceNoteInner] = useMutation(VOTE_ON_FRAGRANCE_NOTE_MUTATION)

  const voteOnFragranceNote = (input: VoteOnFragranceNoteInput) => {
    return wrapQuery(voteOnFragranceNoteInner({ variables: { input } })).map(data => data.voteOnFragranceNote)
  }

  return { voteOnFragranceNote }
}